#!/usr/bin/env python3
# Copyright (c) 2015-2019 The QuantisNet Core developers
...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "quantisnet.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for quantisnetd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if quantisnetd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('quantisnetd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir, num_nodes, cachedir, extra_args=None, redirect_stderr=False):
    ...
    if create_cache:
        ...
        for i in range(MAX_NODES):
            ...
            args = [ os.getenv("EGID", "quantisnetd"), "-server", "-keypool=1", "-datadir="+datadir, "-discover=0", "-mocktime="+str(GENESISTIME) ]
            if i > 0:
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                print("initialize_chain: quantisnetd started, waiting for RPC to come up")
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, redirect_stderr=False):
    ...
    if binary is None:
        binary = os.getenv("EGID", "quantisnetd")
    ...
    bitcoind_processes[i] = subprocess.Popen(args, stderr=stderr)
    if os.getenv("PYTHON_DEBUG", ""):
        print("start_node: quantisnetd started, waiting for RPC to come up")
