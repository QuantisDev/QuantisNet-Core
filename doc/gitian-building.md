...
*Setup instructions for a Gitian build of QuantisNet Core using a Debian VM or physical system.*
...
- [Building QuantisNet Core](#building-nuantisnet-core)
...
Clone the git repositories for QuantisNet Core and Gitian.
...
in the QuantisNet Core repository under 'Fetch and create inputs' to install sources which require
...
To build QuantisNet Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the QuantisNet Core repository.
...
and the quantisnet git repository with the desired tag must both be available locally, and then gbuild must be
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/quantisnet-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/quantisnet-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/quantisnet-osx-build.assert
```
