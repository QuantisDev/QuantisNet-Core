...
#if defined(NDEBUG)
# error "QuantisNet Core cannot be compiled without assertions."
#endif
...
if (nHeight >= consensusParams.nMasternodePaymentsStartBlock) {
        if(phase3){
            ...
            ret = blockReward * 0.8785;
        }
        else if (phase5){
            ret = blockReward * 0.8325;
        }
        else if (phase6){
            ret = blockReward * 0.8785;
        }
            ...
            ret = blockReward * 0.74;
        }
        else if (phase9){
            ret = blockReward * 0.69375;
        }
        else if (phase10){
            ret = blockReward * 0.8325;
        }
        else if (phase11){
            ret = blockReward * 0.4625;
        }
        else if (phase12){
            ret = blockReward * 0.69375;
        }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(QUAN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*(block.CoinBase()), pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(QUAN): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckProof, bool fCheckMerkleRoot)
{
    ...
    if (block.IsProofOfStake()) {
        // CoinStake is a subset of CoinBase in QuantisNet
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(QUAN): spork is off, skipping transaction locking checks\n");
    }
...
bool IsPoSEnforcedHeight(int nBlockHeight) {
    if(fDebug)LogPrintf("POS StartHeight as per vars= %d\n",nFirstPoSBlock);

    return nBlockHeight >= 160;
}
