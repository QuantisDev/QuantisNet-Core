...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a QuantisNet address (e.g. %1)").arg(
...
void setupIPWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setFont(fixedPitchFont());
#if QT_VERSION >= 0x040700
    widget->setPlaceholderText(QObject::tr("Enter a VPS IP (e.g. %1)").arg("127.0.0.2:9801"));
#endif
}
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("quantisnet"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("quantisnet://", Qt::CaseInsensitive))
    {
        uri.replace(0, 9, "quantisnet:");
    }
...
{
    QString ret = QString("quantisnet:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "QuantisNet Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "QuantisNet Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("QuantisNet Core (%s).lnk", chain);
}
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "quantisnetcore.desktop";
    return GetAutostartDir() / strprintf("quantisnetcore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=QuantisNet Core\n";
        else
            optionFile << strprintf("Name=QuantisNet Core (%s)\n", chain);
        #ifdef QUANTISNET_ENABLE_TESTNET_60X
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the QuantisNet Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add QuantisNet Core app to startup item list
