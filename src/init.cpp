...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "quantisnet (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork, stake)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    //strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all QuantisNet specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N QUAN anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
/** Sanity checks
 *  Ensure that QuantisNet Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("QuantisNet Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single QuantisNet Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (!sporkManager.SetSporkAddress(GetArg("-sporkaddr", Params().SporkAddress())) && GetTime() > 1556488204 ) //IGnore error during testing till Monday, April 29, 2019 12:50:04 AM GMT+03:00 KW time
    ...
    //lite mode disables all QuantisNet-specific functionality
    fLiteMode = false; // GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all QuantisNet-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/QuantisDev/QuantisNet-Core/pull/1817 and https://github.com/QuantisDev/QuantisNet-Core/pull/1743
    ...
    //
    // That does not apply to regular quantisnetd use cases, but is quite common for desktop usage.
    ...
    // ********************************************************* Step 11c: update block tip in QuantisNet modules
    ...
    // ********************************************************* Step 11d: start quantisnet-privatesend thread
