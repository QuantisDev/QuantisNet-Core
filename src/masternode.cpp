...
void CMasternode::Check(bool fForce)
{
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        // part 1: expire based on quantisnetd ping
...
bool CMasternode::IsValidNetAddr(CService addrIn)
{
    ...
    return Params().NetworkIDString() == CBaseChainParams::REGTEST ||(addrIn.IsIPv4() || (addrIn.IsIPv6() && IsReachable(addrIn) && addrIn.IsRoutable()));
}
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 10000 QUAN, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
