...
*
*   Why is this needed?
*   - In QuantisNet some blocks are superblocks, which output much higher amounts of coins
...
bool phase6 = nHeight >525600 && nHeight<=700800;
bool phase7 = nHeight >700800 && nHeight<=876000;
bool phase8 = nHeight >876000 && nHeight<=1051200;
bool phase9 = nHeight >1051200 && nHeight<=1226401;
...
bool phase12 = nHeight >1576801;
    ...
    //Convert it to COIN
    backbonefee = backbonefee * COIN;
    CScript devfeepayee;
       CBitcoinAddress devfeeaddr(consensusParams.DevFeeAddr.c_str());

devfeepayee = GetScriptForDestination(devfeeaddr.Get());
    // verify the QuantisNet Backbone address payment
    bool isBackboneRewardValueMet = false;
    for (auto const & i : block.CoinBase()->vout)
    {
        if ((i.scriptPubKey == devfeepayee) && (i.nValue >= backbonefee))
        {
    ...
    {
        LogPrint("gobject", "IsBlockValueValid -- coinbase does not pay correct QuantisNet Backbone Amount\n");
...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutBackboneRet, CTxOut& txoutMasternodeRet, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    // add the QuantisNet Backbone payment
    mnpayments.FillBlockBackbonePayment(txNew, txoutBackboneRet,nBlockHeight);
...
*
*   Add the QuantisNet Backbone payment transaction to the block
*/
void CMasternodePayments::FillBlockBackbonePayment(CMutableTransaction& txNew, CTxOut& txoutBackboneRet,CAmount blockHeight) const
{
auto const & consensus = Params().GetConsensus();
int nHeight = blockHeight;
bool phase1 = nHeight > 1 && nHeight<=300;
//POS Starts here
bool phase2 = nHeight > 300 && nHeight<=2500;
bool phase3 = nHeight >2500 && nHeight<=12500;
...
bool phase12 = nHeight >1576801;
    ...
    CAmount backbonePayment = backbonefee * COIN;

    // split reward between QuantisNet Backbone, masternodes & miners
    txNew.vout[0].nValue -= backbonePayment;
    CScript devfeepayee;
       CBitcoinAddress devfeeaddr(consensus.DevFeeAddr.c_str());

devfeepayee = GetScriptForDestination(devfeeaddr.Get());
    txoutBackboneRet = CTxOut(backbonePayment, devfeepayee);
    txNew.vout.push_back(txoutBackboneRet);
    CTxDestination txdestBackbone;
    if (ExtractDestination(devfeepayee, txdestBackbone))
    {
    ...
    {
        error("CMasternodePayments::FillBlockBackbonePayment -- QuantisNet Backbone script is not valid.");
    }
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f QUAN\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
