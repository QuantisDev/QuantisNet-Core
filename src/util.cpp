...
const char * const BITCOIN_CONF_FILENAME = "quantisnet.conf";
const char * const BITCOIN_PID_FILENAME = "quantisnetd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "quantisnet" is a composite category enabling all Dash-related debug output
                if(ptrCategory->count(std::string("quantisnet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QuantisNetCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QuantisNetCore
    // Mac: ~/Library/Application Support/QuantisNetCore
    // Unix: ~/.quantisnetcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QuantisNetCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QuantisNetCore";
#else
    // Unix
    return pathRet / ".quantisnetcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty quantisnet.conf if it does not excist
