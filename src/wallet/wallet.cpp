...
bool CWallet::SelectStakeCoins(StakeCandidates& setCoins, CAmount nTargetAmount) const
{
    ...
    auto min_age = sporkManager.GetSporkValue(SPORK_18_DISABLE_IPV6_MNS) >= curr_time ? Params().MinStakeAgeNew() : Params().MinStakeAgeOld();
...
bool CWallet::MintableCoins()
{
    ...
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, true);
    auto min_age = sporkManager.GetSporkValue(SPORK_18_DISABLE_IPV6_MNS) >= GetAdjustedTime() ? Params().MinStakeAgeNew() : Params().MinStakeAgeOld();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 QUAN."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
